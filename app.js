...
function renderLocationLinks(location, latLng = "") {
  ...
  locationOutput.innerHTML = `<p><strong>Booth help for ${location}${latLng ? ` (pin ${latLng})` : ""}</strong></p>
...
useLocationBtn.addEventListener("click", () => {
  ...
  locationOutput.innerHTML = "<p>Fetching your current pin...</p>";
  navigator.geolocation.getCurrentPosition(
    ...
    () => {
      locationOutput.innerHTML = "<p>Couldn't get location permission. You can still search by city/area.</p>";
    }
  );
});
...
chatForm.addEventListener("submit", (e) => {
  ...
  if (!msg) return;
  chatLog.innerHTML += `<article class="bubble user">${msg}</article><article class="bubble assistant">${respondToChat(msg)}</article>`;
  chatInput.value = "";
